...
def _fallback(state, questions):
    try:
        ...
        interesting = max(0, min(10, 4 * corr + (2 if state.get("pre_post") else 0)))
        unexpected = max(0, min(10, 5 * corr))
...
def _openai_fallback(state):
    result = chat_json(FALLBACK_JUDGE_SYSTEM, json.dumps(state), model=env("WARSIGNAL_CHEAP_MODEL", "gpt-5-mini"))
