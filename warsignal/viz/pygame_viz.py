...
def _draw_wrapped(surface, font, text, color, rect, line_gap=3):
    y = rect.top
    for line in textwrap.wrap(str(text), width=max(1, rect.width // max(font.get_linesize(), 1))):
...
def _draw_timeseries(surface, pygame, font, rect, panel, series, start, end, result, show_events):
    ...
    if show_events:
        ...
        for _, row in timeline.iterrows():
            ...
            if start <= day <= end:
                x = _x(day, start, end, rect)
                pygame.draw.line(surface, (255, 209, 102), (x, rect.top), (x, rect.bottom), 1)
    war_x = _x(pd.Timestamp(WAR_START), start, end, rect)
    pygame.draw.line(surface, (247, 37, 133), (war_x, rect.top), (war_x, rect.bottom), 1)
    title = font.render(panel.get("note", "Timeseries"), True, PALETTE["text"])
    surface.blit(title, (rect.left, rect.top - font.get_height() - 4))
...
def _draw_scatter(surface, pygame, font, rect, panel, series):
    ...
    surface.blit(font.render(f"{names[0]} vs {names[1]}", True, PALETTE["text"]), (rect.left, rect.top - font.get_height() - 4))
...
def _draw_lagcorr(surface, pygame, font, rect, panel, result):
    ...
    if not lags:
        return
    maximum = max(1.0, max(abs(float(value)) for value in values if value is not None and np.isfinite(value)))
    ...
    for index, (lag, value) in enumerate(zip(lags, values)):
        ...
        surface.blit(label, (x, rect.bottom - label.get_height()))
...
def _draw_sidebar(surface, pygame, fonts, result, series):
    ...
    y = _draw_wrapped(surface, body_font, result.get("hypothesis", ""), PALETTE["text"],
                       pygame.Rect(x, y, SIDEBAR_WIDTH - 30, 88))
    y += 8
    for key in ("validity", "interestingness", "unexpectedness"):
        value = float(result.get("scores", {}).get(key) or 0)
        surface.blit(body_font.render(f"{key}: {value:.1f}", True, PALETTE["text"]), (x, y))
    ...
    stats = [f"r: {corr}", f"best lag: {lag}", f"perm p: {result.get('stats', {}).get('perm_p')}", f"n: {result.get('n_obs', result.get('stats', {}).get('n_obs', 0))}"]
    y += 4
    for line in stats:
...
def render(spec, mission_result_json_path, screenshot_path=None, interactive=True):
    ...
    title_font = pygame.font.SysFont("dejavusans", 17, bold=True)
    ...
    while running:
        ...
        height = max(100, HEIGHT // max(1, len(panels)))
        for index, panel in enumerate(panels):
            rect = pygame.Rect(24, index * height + 40, PLOT_WIDTH - 48, height - 58)
            kind = panel.get("kind")
            if kind == "timeseries":
                current = dict(panel)
        ...
        if hover and 0 <= hover[0] < PLOT_WIDTH and 0 <= hover[1] < HEIGHT:
            ...
            day = start + (end - start) * (hover[0] - 24) / max(PLOT_WIDTH - 48, 1)
            tip = body_font.render(day.date().isoformat(), True, PALETTE["text"])
            screen.blit(tip, (min(hover[0] + 8, PLOT_WIDTH - 150), min(hover[1] + 8, HEIGHT - 24)))
        pygame.display.flip()
        if not interactive:
            target = screenshot or Path("missions/reports") / f"{_mission_id(mission_result_json_path, result)}.png"
            target.parent.mkdir(parents=True, exist_ok=True)
