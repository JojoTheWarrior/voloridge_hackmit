...
interface Seed {
  ...
  error?: string
  report?: ReportText
  explorer?: Pick<Explorer, 'title' | 'description'>
}

// Devin was asked for the report a few minutes after it concluded, and for the explorer after that.
const REPORT_DELAY_MS = 4 * 60_000
const EXPLORER_DELAY_MS = 9 * 60_000

const HOUSTON_EXPLORER = {
  title: 'Houston heat and Henry Hub, day by day',
  description: 'Scrub through the 120 days and select any date to see the temperature anomaly beside the price two days later.',
}
...
export function seedMissions(): Mission[] {
  return SEEDS.map(({ beats = SCRIPT.length, shape, error, report, explorer, ...mission }) => {
    ...
    const settledAt = events.at(-1)!.at
    const later = (delayMs: number) => new Date(Date.parse(settledAt) + delayMs).toISOString()
    const seeded: Mission = { ...mission, createdAt, updatedAt: settledAt, reportPending: false, explorerPending: false, events }
    if (report) {
      const conclusion = events.findLast((e) => e.kind === 'conclusion')
      seeded.report = { ...report, stats: conclusion?.stats ?? [], generatedAt: later(REPORT_DELAY_MS) }
      events.push({ id: 'report', at: seeded.report.generatedAt, kind: 'report' })
    }
    if (explorer) {
      seeded.explorer = { ...explorer, version: 1, src: explorerSrc(mission.id, 1), builtAt: later(EXPLORER_DELAY_MS) }
      events.push({ id: 'explorer', at: seeded.explorer.builtAt, kind: 'explorer' })
    }
    return { ...seeded, updatedAt: events.at(-1)!.at }
  })
}
