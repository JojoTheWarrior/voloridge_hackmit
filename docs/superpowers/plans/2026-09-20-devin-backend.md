...
## Addendum: mission explorer

Some findings are places, not statistics: houses, clinics, landfills. For those a mission gets a third stage, **Thread → Report → Explorer**: a small interactive site that Devin itself writes for that mission (a map to pan, a roof to click, a score, the evidence), shown inside the mission page. It is triggered per mission by the user; most missions will never have one.

Devin writes arbitrary HTML and JavaScript. What keeps it looking like Kingdom is the **explorer kit** in `server/explorer_kit/`: our stylesheet and fonts with light and dark tokens (`kit.css`), optional helpers (`kit.js`: theme sync, a MapLibre map with our basemaps, side panel, ranked list, threshold slider, layer toggle), one complete worked example (`index.html` + `data.json`), and `GUIDE.md`, the instructions Devin reads. The kit is a design system and a starting point, not a widget: Devin may restructure or replace the example entirely.

Verified live on 2026-09-20: Devin can attach files to its session and they appear in `GET …/sessions/{id}/attachments` within seconds. The `url` in that response (`https://app.devin.ai/attachments/{attachment_id}/{name}`) is a web-app link that rejects API keys. The working download is `GET https://api.devin.ai/v3/organizations/{org}/attachments/{attachment_id}/{name}` with the bearer key, which answers `307` to a pre-signed S3 URL; follow it **without** the Authorization header. Devin's chat message for an attachment ends with a raw line `ATTACHMENT:{"url":…,"fileSize":…}`, which must not reach the thread.

### Contract (frozen; `web/src/types.ts` and `web/src/api/index.ts` are already updated)

```
POST /api/missions/:id/explorer            {"instructions"?: string} -> 202 {}
GET  /api/missions/:id/explorer/:version/<path>   -> the stored file
```

- **Request.** Sends Devin one message in the mission's existing session: `EXPLORER_REQUEST` from `server/brief.py`, which embeds `GUIDE.md` and the full text of every kit file, plus the user's `instructions` when given (a first build may have none; a change request such as "add a heatmap" is just a build with instructions). Sets `explorerPending`, sets status `working`, adds no `user_message` event. No-op 202 while pending. No session or Devin unreachable → one `error` event, nothing pending, 202. If the mission was `done` it returns to `done` afterwards, exactly as reports do. Blank `instructions` are treated as absent; over 2,000 characters → 422 `{field: "text", message: "Keep instructions under 2,000 characters"}`.
- **Delivery.** Devin attaches exactly one archive named `explorer-v<N>.zip` (N = 1, 2, …, its own counter) and sets `explorer` in `structured_output`: `{version: N, archive: "explorer-v<N>.zip", entry: "index.html", title, description}`. A build counts as delivered when one is pending, `explorer.version` is greater than the stored version (or none is stored), and the named archive is present in the attachment list. The server downloads it, unpacks it, stores it, clears pending, and upserts the single `{kind: "explorer"}` event (id `explorer`) at the end of the thread (after `report` if present). Pending for more than 15 minutes → `error` event "The explorer did not arrive", pending cleared. A present-but-invalid archive → `error` event naming the reason, pending cleared.
- **Unpacking is hostile-input handling.** Reject: more than 200 entries; more than 40 MB unpacked or any entry over 25 MB; compression ratio over 100; absolute paths, `..` segments, backslashes, symlinks, or names that escape the target; any extension outside `html css js mjs json geojson csv txt png jpg jpeg webp gif svg woff2`; a missing `entry`. A single top-level folder wrapping everything is stripped. Anything under `kit/` inside the archive is discarded (ours is served instead). Files land in `<db dir>/explorers/<mission id>/<version>/`; older versions are kept.
- **Serving.** `GET …/explorer/:version/<path>` serves the stored file; any path under `kit/` serves the canonical file from `server/explorer_kit/` instead, so design fixes reach every explorer ever built. Unknown mission, version or path → 404; path traversal → 404. Every response carries `Access-Control-Allow-Origin: *` (the frame has an opaque origin, so its own relative fetches are cross-origin), `X-Content-Type-Options: nosniff`, `Cache-Control: no-store`, and on HTML and SVG documents a `Content-Security-Policy` beginning `sandbox allow-scripts allow-popups allow-popups-to-escape-sandbox` whose `default-src`, `script-src`, `style-src` and `font-src` name the server's own origin as the browser addressed it (taken from the validated Host header), plus `data: blob:`, `'unsafe-inline'`, and the CDNs `unpkg.com`, `cdn.jsdelivr.net`, `fonts.googleapis.com` / `fonts.gstatic.com`; `img-src * data: blob:; connect-src *; worker-src blob:; child-src blob:`. It must **not** use `'self'`: a document sandboxed without `allow-same-origin` has an opaque origin, against which Chrome matches `'self'` to nothing, blocking the explorer's own kit and data files (found in integration). See `explorer_csp` in `server/app.py`, so an explorer opened directly is sandboxed exactly as it is in the frame.
- **Mission JSON.** `GET /api/missions/:id` always includes `explorerPending` and includes `explorer` (`version`, `title`, `description`, `src`, `builtAt`) only when a build exists; `src` is `/api/missions/<id>/explorer/<version>/<entry>`. Summaries are unchanged.
- **The frame.** The web app shows `explorer.src + '?theme=light|dark'` in `<iframe sandbox="allow-scripts allow-popups allow-popups-to-escape-sandbox">` (never `allow-same-origin`) and posts `{type: 'kingdom:theme', theme}` to it whenever the theme changes. `kit.js` applies both.
- **Navigation.** A mission has tabs in its header: Thread, Report, Explorer (`/missions/:id`, `/missions/:id/report`, `/missions/:id/explorer`). The "Generate report" and "Build explorer" actions live in their tab's empty state rather than as header pills; the header keeps the status dot, title, status text, the Devin link and "Mark done".
- **Demo mode.** FakeDevin answers an explorer request two beats later with a real zip built from the kit's worked example (`index.html` + `data.json`), bumping the version on every request and appending the instructions to the description so a change is visible, plus one message "The explorer is ready."
