...
- **Serving.** `GET …/explorer/:version/<path>` serves the stored file; any path under `kit/` serves the canonical file from `server/explorer_kit/` instead, so design fixes reach every explorer ever built. Unknown mission, version or path → 404; path traversal → 404. Every response carries `Access-Control-Allow-Origin: *` (the frame has an opaque origin, so its own relative fetches are cross-origin), `X-Content-Type-Options: nosniff`, `Cache-Control: no-store`, and on HTML documents `Content-Security-Policy: sandbox allow-scripts allow-popups allow-popups-to-escape-sandbox; default-src 'self' data: blob:; script-src 'self' 'unsafe-inline' https://unpkg.com https://cdn.jsdelivr.net; style-src 'self' 'unsafe-inline' https://unpkg.com https://cdn.jsdelivr.net https://fonts.googleapis.com; font-src 'self' data: https://fonts.gstatic.com; img-src * data: blob:; connect-src *; worker-src blob:; child-src blob:` so an explorer opened directly is sandboxed exactly as it is in the frame.
