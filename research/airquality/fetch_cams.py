...
st = pd.read_csv("stations_sample.csv", dtype={"id": str})[["lat", "lon"]]
ci = pd.read_csv("cities_coverage.csv", keep_default_na=False); ci = ci[ci["pop"].astype(int) >= 500_000][["lat", "lon"]].astype(float)
p = pd.concat([st, ci]); i = np.rint((p.lat + 90) / 0.4).astype(int).clip(0, 450); j = (np.rint((p.lon + 180) / 0.4).astype(int)) % 900
...
def work(chunk):
    ...
    np.save(f"cams/{VAR}_{chunk}.npy", out)
    return chunk
if __name__ == "__main__":
    import os; os.makedirs("cams", exist_ok=True)
    pd.DataFrame(cells, columns=["i", "j"]).to_csv("cams/cells.csv", index=False)
    print(len(cells), "cells", len(blocks), "blocks", flush=True)
    todo = [c for c in CHUNKS if not os.path.exists(f"cams/{VAR}_{c}.npy")]
