...
def orbit_files(day):
    ...
    for n in names:
        ...
        if 15 <= h <= 22: out.append((m.group(1), n))
    return out
...
np.savez_compressed(f"s5p_windows_{start}_{end}.npz", plant=ids, stamp=stamps, no2=np.stack([r[2] for r in allrecs]), qa=np.stack([r[3] for r in allrecs]))
