...
WMS="https://gibs.earthdata.nasa.gov/wms/epsg4326/best/wms.cgi"; LAYER='VIIRS_NOAA20_Clear_Sky_Confidence_Night'
BBOX=(36.0,18.0,64.0,40.0); RES=0.02; W=int((BBOX[2]-BBOX[0])/RES); H=int((BBOX[3]-BBOX[1])/RES)
def lut():
    ...
    return t
T=lut(); out=Path('data/cloud_gulf'); out.mkdir(exist_ok=True,parents=True)
def job(d):
    f=out/f'{d}.npy'
    if f.exists(): return
    for a in range(4):
...
with ThreadPoolExecutor(6) as ex: list(ex.map(job,dates))
print('done')
